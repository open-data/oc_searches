...
def filter_csv_record(csv_record,search: Search, fields: dict, codes: dict, format: str):
    redundant_fields = ['indicator_en', 'indicator_fr', 'indicator_deadline_en',
                        'indicator_deadline_fr', 'indicator_lead_dept']
    # This is redundant if the nap6_preprocess_csv.py command is run, but is here as a precaution
...
def load_csv_record(csv_record: dict, solr_record: dict, search: Search, fields: dict, codes: dict, format: str):
    ...
    if indicator in codes['indicators']:
        solr_record['lead_dept'] = codes['indicators'][indicator].extra_01
