...
class Command(BaseCommand):
    help = 'Process the NAP 6 Commitments files for loading into Solr'
    ...
    # Process the records in the CSV file one at a time
        cur.execute(f'DROP TABLE IF EXISTS nap6')
        ...
        try:
            for chunk in pd.read_csv(options['csv'], chunksize=500, delimiter=","):
                ...
                chunk.to_sql(name="nap6", con=conn, if_exists='append')

            cur.execute("insert into latest select indicators, max(reporting_period) from nap6 group by indicators")
            cur.execute("commit")

            cur.execute("select l.reporting_period, n.reporting_period, n.commitments, n.milestones, n.indicators, n.status, n.progress_en, n.progress_fr, n.evidence_en, n.evidence_fr, n.challenges_en, n.challenges_fr, n.owner_org, n.owner_org_title from nap6 n join latest l on n.indicators = l.indicators")
